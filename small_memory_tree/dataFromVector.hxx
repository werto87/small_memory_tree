...
template <typename T>
std::optional<T>
childWithValue (std::vector<T> const &vec, uint64_t index, T value, T markerForEmpty)
{
  for (auto i = uint64_t{ 1 }; i <= boost::numeric_cast<uint64_t> (maxChildren (vec, markerForEmpty)); i++)
    {
      if constexpr (TupleLike<T>)
        {
          if (vec[boost::numeric_cast<uint64_t> (std::get<0> (vec[index + i]))+index + i] == value)
            {
              return vec[index + i];
            }
        }
        ...
        {
          if (vec[boost::numeric_cast<uint64_t> (vec[index + i])+index + i] == value)
            {
              return vec[index + i];
            }
        }
