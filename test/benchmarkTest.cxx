...
TEST_CASE ("SmallMemoryTreeNew calcChildrenForPath", "[!benchmark]")
{
  SECTION ("few elements")
  {
    auto tree = st_tree::tree<int>{};
    ...
    {
      auto smallMemoryTree = SmallMemoryTreeNew<int, uint8_t>{ StTreeAdapter{ tree } };
  ...
  SECTION ("max children == 100")
  {
    ...
    {
      auto smallMemoryTree = SmallMemoryTreeNew<uint64_t, uint8_t>{ StTreeAdapter{ tree } };
      BENCHMARK ("SmallMemoryTreeNew") { return calcChildrenForPath (smallMemoryTree, std::vector<uint64_t>{ 0, 1 }); };
    }
    SECTION ("calcChildrenForPath 0 99")
    {
      auto smallMemoryTree = SmallMemoryTreeNew<uint64_t, uint8_t>{ StTreeAdapter{ tree } };
      BENCHMARK ("SmallMemoryTreeNew") { return calcChildrenForPath (smallMemoryTree, std::vector<uint64_t>{ 0, 99 }); };
    }
  }
  SECTION ("max children == 1000")
  {
    ...
    auto smallMemoryTree = SmallMemoryTreeNew<uint64_t, uint64_t>{ StTreeAdapter{ tree } };
    SECTION ("calcChildrenForPath 0 0")
    {
      BENCHMARK ("SmallMemoryTreeNew") { return calcChildrenForPath (smallMemoryTree, std::vector<uint64_t>{ 0, 0 }); };
    }
    SECTION ("calcChildrenForPath 0 999")
    {
      BENCHMARK ("SmallMemoryTreeNew") { return calcChildrenForPath (smallMemoryTree, std::vector<uint64_t>{ 0, 999 }); };
    }
  }
  SECTION ("max children == 10000")
  {
    ...
    {
      BENCHMARK ("SmallMemoryTreeNew") { return calcChildrenForPath (smallMemoryTree, std::vector<uint64_t>{ 0, 9999 }); };
    }
    SECTION ("calcChildrenForPath 0 9999 with binary find")
    {
      BENCHMARK ("SmallMemoryTreeNew") { return calcChildrenForPath (smallMemoryTree, std::vector<uint64_t>{ 0, 9999 }, true); };
    }
  }
}